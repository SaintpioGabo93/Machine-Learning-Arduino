...
def layers(layer):
    ...
    for k in range(len(layer)):
        ...
        if layer[k].get_config()['activation'] == 'softmax':
            capa = "double aux1 = 0;\nfor(int i = 0 ; i<" + str(rowW) + "; i++ ) {aux=0.0;for(int j = 0 ; j <" + str(
                colW) + " ; j++ )"
            capa = capa + "{ aux=aux+W" + str(k + 1) + "[i][j]*a" + str(k) + "[j];} a" + str(
                k + 1) + "[i]=(aux+b" + str(k + 1) + "[i]);aux1=aux1+exp(a" + str(k + 1) + "[i]);}"
            capa = capa + "\ndouble minimo = 0.0;"
            capa = capa + "\nint classes = 0;"
            capa = capa + "\nfor(int i = 0;  i<" + str(rowW) + "; i++){a" + str(k + 1) + "[i] = exp(a" + str(
                k + 1) + "[i])/aux1;" + "if(a" + str(k + 1) + "[i]>minimo){minimo=a" + str(k + 1) + "[i];classes=i;}}"
        else:
            capa = "for(int i = 0 ; i<" + str(rowW) + "; i++ ) {aux=0.0;for(int j = 0 ; j <" + str(
                colW) + " ; j++ ) { aux=aux+W" + str(k + 1) + "[i][j]*a" + str(k) + "[j];} a" + str(k + 1) + "[i]=" + \
                   layer[k].get_config()['activation'] + "(aux+b" + str(k + 1) + "[i]);}"
